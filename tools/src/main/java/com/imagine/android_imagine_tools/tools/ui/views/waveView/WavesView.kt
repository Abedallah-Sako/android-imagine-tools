package com.imagine.android_imagine_tools.tools.ui.views.waveView
...
class WavesView @JvmOverloads constructor(
    ...
    private val defStyleAttr: Int = R.style.Widget_WaveView
    ...
    private val gradientColors =
        intArrayOf(modifyAlpha(green, 0.50f), modifyAlpha(green, 0.10f), modifyAlpha(green, 0.05f))
    ...
    private fun createStarPath(
        ...
        val angleInRadians =
            2.0 * Math.PI / mPoints // essentially 360/20 or 18 degrees, angle each line should be drawn
        ...
        for (i in 1 until mPoints) {
            ...
            val nextPointX =
                center.x + (hypotenuse * Math.cos(startAngleInRadians - angleInRadians * i)).toFloat()
            val nextPointY =
                center.y + (hypotenuse * Math.sin(startAngleInRadians - angleInRadians * i)).toFloat()
