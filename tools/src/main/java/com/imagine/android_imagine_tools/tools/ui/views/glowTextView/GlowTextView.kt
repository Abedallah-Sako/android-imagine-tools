package com.imagine.android_imagine_tools.tools.ui.views.glowTextView
...
class GlowTextView @JvmOverloads constructor(
    ...
    private fun setViewAttributes() {
        ...
        startColorAlpha =
            ((arr.getFloat(R.styleable.GlowTextView_startColorAlpha, 1f)) * 255).toInt()
        ...
        if (arr.getResourceId(R.styleable.GlowTextView_fontFamily, 0) != 0) {
            typeFace = ResourcesCompat.getFont(
                context,
                arr.getResourceId(R.styleable.GlowTextView_fontFamily, 0)
            )
        }
    ...
    private fun drawGlowBackground(glowPaint: Paint, x: Float, y: Float, canvas: Canvas?) {
        glowPaint.shader =
            shaderFactory(x + paddingStart, y + (glowTextView.layout.height / 2) + paddingTop)
