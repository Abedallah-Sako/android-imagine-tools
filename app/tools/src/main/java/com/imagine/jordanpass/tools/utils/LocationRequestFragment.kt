        ...
        if (isGranted) {
            ...
            if (Build.VERSION.SDK_INT >= 23) {
                if (this.shouldShowRequestPermissionRationale(Manifest.permission.ACCESS_FINE_LOCATION)) {
                    Toast.makeText(
                        this.requireContext(),
                        com.imagine.jordanpass.tools.R.string.location_permission_warning,
                        Toast.LENGTH_SHORT
                    ).show()
                    locationCoordinatesCallback?.invoke(Pair(null,false))
                    removeFragment()
...
//                    ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.ACCESS_FINE_LOCATION}, 1);
    ...
    private fun showPopUp(activity: Activity) {
        AlertDialog.Builder(this.requireContext())
            .setTitle(com.imagine.jordanpass.tools.R.string.location_permission_popup_title)
            .setMessage(com.imagine.jordanpass.tools.R.string.location_permission_rationale)
...
//                    locationsPermissionLauncher.launch(Manifest.permission.ACCESS_FINE_LOCATION)
    ...
    private fun showSettingsPopUpForLocation() {
        AlertDialog.Builder(this.requireContext())
            .setTitle(com.imagine.jordanpass.tools.R.string.location_permission_popup_title)
            .setMessage(com.imagine.jordanpass.tools.R.string.location_permission_settings)
            .setPositiveButton(com.imagine.jordanpass.tools.R.string.settings_tab,
                DialogInterface.OnClickListener { dialog, which -> openAppSettings() })
        ...
        mLocationCallback = object : LocationCallback() {
            ...
            override fun onLocationResult(locationResult: LocationResult) {
                locationResult.locations.forEach { location ->
                    location?.let {
                        locationCoordinatesCallback?.invoke(Pair(LatLng(location.latitude,location.longitude),true))
                        removeFragment()
                    }
                }
            }
